...
CREATE POLICY IF NOT EXISTS "Riders can view own stock movements"
...
CREATE POLICY IF NOT EXISTS "Riders can confirm own stock movements"
ON public.stock_movements
FOR UPDATE
USING (
  has_role('rider'::user_role)
...
CREATE POLICY IF NOT EXISTS "Riders can insert own inventory"
ON public.inventory
FOR INSERT
WITH CHECK (
  has_role('rider'::user_role)
...
CREATE POLICY IF NOT EXISTS "Riders can update own inventory"
ON public.inventory
FOR UPDATE
USING (
  has_role('rider'::user_role)
